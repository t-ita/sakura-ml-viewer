...
└── MainLayout                        # 認証済みのみ。1カラム: 上部固定バー + カード一覧
    ├── ControlPanel                  # 上部バー
    │   ├── Branding                  # ロゴ or タイトル文字列（§6.6）
    │   ├── FilterChips               # 適用中の条件チップ＋絞り込み解除ボタン（適用中のみ）
    │   ├── SearchDialog              # 「投稿絞り込み」ボタンと検索ダイアログ（§6.3）
...
`Button` `Input` `Label` `Card`（記事カード） `Dialog`（検索・パスワード変更） `DropdownMenu` `Calendar`（日付範囲。検索ダイアログ内に直接配置） `Skeleton`（ローディング） `Badge`（添付・条件チップ） `Alert`（エラー・空状態） `Separator` `Sonner`（トースト通知） `Tooltip`。
