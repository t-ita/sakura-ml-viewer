...
依存ゼロの軽量テストランナー。`mail_parser.php`（fmlメールパーサ、fixtureメール5種）、`actives.php`（activesファイル解析）、`api.php`（LIKE検索エスケープ・日付境界・スニペット生成・記事番号範囲`id_from`/`id_to`）、`auth.php`（トークン生成・ハッシュ・ダミーパスワードハッシュの永続化）、`db.php`（マイグレーションの冪等性）、`ratelimit.php`（レート制限ロジック）をカバーする。実行の都度 `backend/tests/tmp/` に一時SQLite DBを作り直すため、外部状態に依存しない。
